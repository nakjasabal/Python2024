'''
지역변수와 전역변수 
: 2개의 변수가 동일한 이름으로 사용되는 경우 지역변수가 항상
높은 우선순위를 가진다. 
'''
#전역변수로 선언. 해당 프로그램 전체에서 사용할 수 있음 
total = 0  
#함수 선언 
def sum(arg1, arg2):   
    #매개변수를 통해 받은 2개의 값을 합산하여 지역변수에 할당 
    total = arg1 + arg2
    #지역변수 total은 해당 함수내에서만 사용할 수 있다. 
    print("지역변수 total=", total)
    #계산된 값을 호출한 지점으로 반환한다. 
    return total  
...
def defaultArgs(lan="Java"):
    ...
    return  
#이처럼 인수가 전달되지 않으면 'Java'가 사용된다.
defaultArgs()  
#값을 전달하면 디폴트값은 사용되지 않는다. 
defaultArgs("C++")  
 
'''
4.가변매개변수
: 여러개의 매개변수를 전달하는 경우 그 갯수가 매번 달라질 수 있는
경우에 사용하는 방식으로
    *를 이용해서 매개변수의 값을 튜플로 받는다. 
    **를 이용해서 매개변수의 값을 딕셔너리로 받아 함수 내부에서
    사용한다. 
'''
def product(*args):
    #전달되는 인수를 튜플로 받아서 출력 
    ...
    #튜플의 크기만큼 반복할 수 있다. 
    for a in args:
        #인자의 갯수만큼 누적곱을 계산하여 result에 저장한다.
        result *= a
    #계산된 결과를 호출한 지점으로 반환한다. 
    return result
#4개의 인자를 전달하여 함수를 호출한다. 전달된 값은 튜플로 변환된다.
print("product(1,2,3,4) =", product(1,2,3,4))

# **를 통해 딕셔너리로 인수를 받을 수 있다. 
def famousMan(**man):  
    print('**man', man)
