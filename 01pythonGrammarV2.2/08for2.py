...
#입력한 이전 연도(작년)까지의 전체날짜를 누적해서 더해준다.  
for d in range(1,year):
    if d % 400 == 0: #400으로 나눠서 떨어지면 윤년
        total_days = total_days + 366
    elif d % 100 == 0: #100으로 나눠서 떨어지면 평년 
...
'''
입력월이 3이상이고, 입력년도가 윤년일경우 1을 더해줘야 한다. 
윤년은 2월 29일까지 있기때문이다. 만약 1월, 2월이라면 이 부분은 
고려할 필요가 없다. 
'''
if month >= 3:
    if year % 400 == 0:
...
#앞에서 누적한 날짜에 내가 입력한 날짜를 더해준다.
total_days += day
print()
print("total_days :", total_days)

'''
요일은 항상 일요일부터 시작이므로 7로 나눈 나머지가 0이라면 
일요일로 판단할 수 있다. 
'''
